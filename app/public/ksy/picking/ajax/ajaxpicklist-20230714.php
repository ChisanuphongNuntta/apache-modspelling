...
if($_GET['a'] == 'CheckID') {
	...
	for ($i = 1; $i <= $ax; $i++){
		...
		switch ($ListDetail['Status'][$RowID[$i]]){
			case 2 :
				$BGColorAndTextColor = "background-color: #FFFF99; color: #000000;";//ส่งเท่าทีมี (เช็คหมายเหตุด้วยว่า ตัดออกเอาเท่าไหร)
				break;
			case 3 :
				$BGColorAndTextColor = "background-color: #EE7F7F; color: #FFFFFF;";//ตัดหมด
				break;
			case 9 :
				$BGColorAndTextColor = "background-color: #D4B6E0; color: #FFFFFF;";//เบิกเพิ่ม
				break;
			default :
							...
							if ($DetailData['Location'] != "" AND $DetailData['OnShelf'] >= intval($_POST['TxtQty']) AND $DetailData['Location'] == $_POST['RacKBar'] AND $OpenData['Qty'] > $OpenData['OpenQty']){
								if ($newOpen > $OpenData['Qty']){
									...
									if ($OnSAP['OnHand'] <  intval($_POST['TxtQty']) AND $wai == 1){
										...
										if ($OpenData['WhsCode'] == 'KSY' AND $wai == 1 ) {
											...
											if($ShowRes['OnHand'] >= intval($_POST['TxtQty'])) {
												...
												$Update_whsquota = "UPDATE whsquota SET OnHand = ".$newRes." WHERE ItemCode = '".$ShowRes['ItemCode']."' AND WhsCode = '".$OpenData['WhsCode']."' AND CH = '".$_POST['CHEntry']."'";
												...
												$sql7 = "SELECT SUM(OnHand) AS OnHand FROM OITW WHERE ItemCode = '".$ShowItem['ItemCode']."' AND WhsCode IN ('KSY','KSM')";
												...
												if($TotalPick >= intval($_POST['TxtQty'])) {
													...
													if ($ShowRes['OnHand'] > 0){
														...
														$UpdateWhs = "UPDATE whsquota SET OnHand = ".$newRes." WHERE ItemCode = '".$ShowRes['ItemCode']."' AND WhsCode = '".$OpenData['WhsCode']."' AND CH = '".$_POST['CHEntry']."'";
...
if($_GET['a'] == 'CutAmount') {
	$Update = "UPDATE picker_soheader 
			   ...
			   WHERE SODocEntry = '".$_POST['SODocEntry']."'";
	MySQLUpdate($Update);
}
		...
		if($ChkRow == 0) {
			...
			if($ChkRow2 != 0) {
				$Update = "UPDATE picker_sodetail 
						   ...
						   WHERE DocEntry = '".$_POST['SODocEntry']."' AND DocType = 'ORDR' AND ItemCode = '".$result['ItemCode']."'";
						//    echo $Update;
				MySQLUpdate($Update);		   
			}
		}
