...
$sql1 = "SELECT T0.DocEntry,T0.DocNum,DATE(T0.DocDate) AS DocDate,T0.DocType,T0.ObjType,T0.Payment_Cond,T0.DocStatus,T0.CardCode,T0.SlpCode,T0.DocTotal,((T0.GrossProfit/(T0.DocTotal-T0.VatSum))*100) AS GP,
				T1.TeamCode,T1.MainTeam
...
if ($_SESSION['DeptCode'] == 'DP003' OR $_SESSION['DeptCode'] == 'DP005' OR $_SESSION['DeptCode'] == 'DP008'){
	...
	if ($CR > 0 ){ // เกินวงเงิน
		...
		$sql1 = "SELECT COUNT(DocEntry) AS OV FROM OINV WHERE DocDueDate < '".date("Y-m-d",strtotime("-30 day",strtotime($DataCus['DocDate'])))."' AND DocStatus = 'O' AND CardCode IN ".$CardList;
		//echo $sql1;
		$getOverDue = SAPSelect($sql1);
		$OverDue = odbc_fetch_array($getOverDue);
		if ($OverDue > 0){//เกินดิว
			$App["CR"] = '1'; 
			$TypeCR = 'D';
			$AppLV = " AND (T0.UserApp = '18' OR T0.UserApp = 'LV057') ";
		}else{
...
if (CHKRowDB($sql1) > 0 && $GPchk == 1 ){
	$sql1 = "SELECT T0.ItemCode,T0.UnitPrice,T1.MgrPrice
	         FROM order_detail T0
			      LEFT JOIN pricelist T1 ON  T0.ItemCode = T1.ItemCode
			 WHERE T1.PriceStatus = 'A' AND T1.PriceType = 'STD' AND DocEntry = '".$DocEntry."' AND Line_SP = 'Y' 
			 ORDER BY T0.UnitPrice";
	$getCHkPrice = MySQLSelectX($sql1);
	while ($CHkPrice = mysqli_fetch_array($getCHkPrice)){
		if ($CHkPrice['UnitPrice'] >= $CHkPrice['MgrPrice'] && $App['GP'] == '0' && $CHkPrice['MgrPrice'] != 0){
			$App['GP'] = 'Mgr';
		}else{
			if ($App['GP'] == 'Mgr'){
				if ($CHkPrice['UnitPrice'] >= $CHkPrice['MgrPrice'] && $CHkPrice['MgrPrice'] != 0){
					$App['GP'] = '1';
				}
			}else{
				$App['GP'] = '1';
