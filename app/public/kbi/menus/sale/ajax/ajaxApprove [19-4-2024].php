...
*/ 
	...
	if ($App['SO'] != '0'){ //อนุมัติ SO
		...
		switch ($DataCus['DocType']){
			case 'SB' :
				$WPhus = " OR T0.ConditionOption = 'SB' ";
				$WPhus2 = " OR T0.TeamCode = 0 ";
				break;
			case 'SA' :
				$WPhus = " OR T0.ConditionOption = 'SA' ";
				$WPhus2 = " OR T0.TeamCode = 0 ";
				break;
			default :
				$WPhus = "  ";
		...
		$sql1 = "SELECT T0.DocType,T0.LvApp,T0.ApproveCode AS AppCode,
				 ...
				 WHERE T0.ApproveCode = 'AppSO' AND (T0.TeamCode = '".$DataCus['TeamCode']."' OR T0.TeamCode = '".$DataCus['MainTeam']."' $WPhus2 ) AND StatusDoc = 'A' ".$WPhus;
		$sql1 .= " ORDER BY T0.LvApp,T0.StepApprove ";
        //echo $sql1;
		$AppSO = MySQLSelectX($sql1);
		$LoopR = 0;
		/***** LOOP ใส่ข้อมูล  */
		while ($row = mysqli_fetch_array($AppSO)){
			if ($AppSO['UserApp'] == '18'){
				switch($_SESSION['DeptCode']){
					case 'DP005':
					case 'DP006':
