...
*/
	...
	if($_POST['filt_team'] == "ALL") {
		...
		switch($_SESSION['uClass']) {
			case 20: $OrderWhr = " AND T3.MainTeam = '".$_POST['filt_team']."' AND (T0.CreateUkey = '".$_SESSION['ukey']."' OR T3.uKey = '".$_SESSION['ukey']."') "; break;
			default: $OrderWhr = " AND T3.MainTeam = '".$_POST['filt_team']."' OR (T0.CreateUkey = '".$_SESSION['ukey']."')"; break;
		}
	} else {
	...
	ORDER BY
        CASE
			WHEN (T0.DraftStatus = 'Y' AND T0.CANCELED = 'N') THEN 1
	...
	if($Rows == 0) {
		...
		$GetSAPEntry = "SELECT GROUP_CONCAT(T0.ImportEntry) AS 'SAPEntry'
		FROM order_header T0
		LEFT JOIN users T1     ON T0.CreateUkey = T1.Ukey
		LEFT JOIN positions T2 ON T1.LvCode     = T2.LvCode
		LEFT JOIN OSLP T3      ON T0.SlpCode    = T3.SlpCode 
		...
		$GetSAPEntry = MySQLSelect($GetSAPEntry);
		$SAPEntry = $GetSAPEntry['SAPEntry'];

		if($SAPEntry != "") {
			$SAPDocSQL = "SELECT T0.DocEntry, T1.BeginStr+CAST(T0.DocNum AS VARCHAR) AS 'DocNum' FROM ORDR T0 LEFT JOIN NNM1 T1 ON T0.Series = T1.Series WHERE T0.DocEntry IN ($SAPEntry)";
			$SAPDocQRY = SAPSelect($SAPDocSQL);
			while($SAPDocRST = odbc_fetch_array($SAPDocQRY)) {
				$DocEty[$SAPDocRST['DocEntry']] = $SAPDocRST['DocNum'];
				//$EntryData[$SAPDocRST['DocEntry']] = 
			}
		}
		...
		while($OrderRST = mysqli_fetch_array($OrderQRY)) {
			...
			if($OrderRST['QQst'] == "Y") {
				$QQtxt = "<span class='badge bg-danger'>ด่วน</span>";
			} else {
				$QQtxt = NULL;
			}
			$int_status = 0;
			...
			if($OrderRST['CANCELED'] == "N" && $OrderRST['DraftStatus'] == "Y" && $OrderRST['DocStatus'] == "O") {
				$int_status = 1; /* บันทึกร่าง */
			}

			if($OrderRST['CANCELED'] == "N" && $OrderRST['DraftStatus'] == "N" && $OrderRST['DocStatus'] == "P" && $OrderRST['AppStatus'] == "P") {
				$int_status = 2; /* เอกสารรอตรวจสอบ */
			}

			if($OrderRST['CANCELED'] == "N" && $OrderRST['DraftStatus'] == "N" && $OrderRST['DocStatus'] == "P" && $OrderRST['AppStatus'] == "Y") {
				$int_status = 3; /* เอกสารผ่านการอนุมัติ */
			}
			
			if($OrderRST['CANCELED'] == "N" && $OrderRST['DraftStatus'] == "N" && $OrderRST['DocStatus'] == "C" && $OrderRST['AppStatus'] == "N") {
				$int_status = 4; /* เอกสารไม่ผ่านการอนุมัติ */
			}

			if($OrderRST['CANCELED'] == "N" && $OrderRST['DraftStatus'] == "N" && $OrderRST['DocStatus'] == "C" && $OrderRST['AppStatus'] == "Y") {
			...
			*/
			...
			if($OrderRST['ImportEntry'] != "" && $int_status == 5 && isset($DocEty[$OrderRST['ImportEntry']])) {
				$arrCol[$i]['SAPDocNum'] = "<a href='javascript:void(0);' class='text-success' onclick=\"PrintSB('".$OrderRST['ImportEntry']."')\">".$DocEty[$OrderRST['ImportEntry']]."</a>";
			} else {
				if($int_status == 5) {
					$arrCol[$i]['SAPDocNum'] = "<strong class='text-danger'>Import Error!</strong>";
				} else {
					$arrCol[$i]['SAPDocNum'] = "";
				}
			...
			if($_SESSION['DeptCode'] != $OrderRST['DeptCode'] && $_SESSION['DeptCode'] != "DP002") {
				switch($int_status) {
					...
					case 2:   $txt_status = "<span class='badge w-100' style='background-color: #C79910; color: #FFF;'><i class='far fa-clock fa-fw fa-lg'></i> รออนุมัติ <b>[".$OrderRST['Approved']."/".$OrderRST['Approve']."]</b></span>"; break;
				...
				switch($int_status) {
					...
					case 2:
						...
						if($OrderRST['Approved'] != 0 && $_SESSION['uClass'] == "LV036") {
							$dis_edit   = " disabled";
						}
						// $dis_import = " disabled";
						break;
			...
			if($int_status != 0) {
				$txt_opt = "<div class='dropdown'>";
					...
					$txt_opt.= "<ul class='dropdown-menu' style='font-size: 13px;'>";
						...
						$txt_opt.= "<li><a href='javascript:void(0);' class='dropdown-item ordr-impt$dis_import' onclick='ExportSO(".$OrderRST['DocEntry'].")'><i class='fas fa-share-square fa-fw fa-1x'></i> นำเข้า SAP</a></li>";
			...
			$arrCol[$i]['DocDate']    = date("d/m/Y",strtotime($OrderRST['DocDate']));
			$arrCol[$i]['DocDueDate'] = date("d/m/Y",strtotime($OrderRST['DocDueDate']));
			$arrCol[$i]['DocumentNo'] = "<a href='javascript:void(0);' onclick='PreviewSO(".$OrderRST['DocEntry'].",$int_status)'>".$OrderRST['DocumentNo']."</a>";
			$arrCol[$i]['CardInfo']   = $OrderRST['CardCode']." ".$OrderRST['CardName']." $QQtxt<br/><small>ผู้จัดทำ: ".$OrderRST['CreateName']."</small>";
			$arrCol[$i]['U_PONo']     = $OrderRST['U_PONo'];
			$arrCol[$i]['DocTotal']   = number_format($OrderRST['DocTotal'],2);
			$arrCol[$i]['MainTeam']   = "<span class='badge bg-dark'>".$OrderRST['MainTeam']."</span>".$OrderRST['SlpName'];
...
if($_GET['p'] == "SOPreview") {
	...
	if($AttRow == 0) {
		...
		while($AttRST = mysqli_fetch_array($AttQRY)) {
			$AttachList .= "<tr>";
				...
				$AttachList .= "<td class='text-center'><a class='btn btn-success btn-sm' href='../FileAttach/SO/".$AttRST['FileDirName'].".".$AttRST['FileExt']."' target='_blank' download='".$AttRST['FileOriName'].".".$AttRST['FileExt']."'><i class='fas fa-file-download fa-fw fa-1x'></i></a></td>";
			$AttachList .= "</tr>";
			$no++;
		}
	}
		...
		case 5:
			...
			if($Row > 0) {
				...
				while($AppRST = mysqli_fetch_array($AppQRY)) {
					...
					/* รอพิจารณา 001 010 011 100 110 111 101 */
	...
	$AppGPSQL = 
		"SELECT
			T1.TaxType, T0.Line_SP, T0.ItemCode, T0.ItemName, T0.ItemStatus, T0.WhsCode, T0.Quantity, T0.UnitMsr, T0.GrandPrice,
			T0.UnitPrice*1.07 AS  UnitPrice,
		...
		WHERE T0.DocEntry = $DocEntry";
	if(ChkRowDB($AppGPSQL) > 0) {
		$AppGPQRY = MySQLSelectX($AppGPSQL);
		$AppGP     = "";
		$SUMPrice  = 0;
		$SUMProfit = 0;
		while($AppGPRST = mysqli_fetch_array($AppGPQRY)) {
			if($AppGPRST['UnitPrice'] * $AppGPRST['Quantity'] == 0) {
				$GP = 0;
			} else {
				$GP = ($AppGPRST['LineProfit'] / ($AppGPRST['LineTotal']+$AppGPRST['LineVatSum'])) * 100;
			}

			if($AppGPRST['Line_SP'] == "Y") {
				$SP = " checked";
			...
			$SUMPrice  = $SUMPrice + ($AppGPRST['LineTotal']+$AppGPRST['LineVatSum']);
			$SUMProfit = $SUMProfit + $AppGPRST['LineProfit'];

			$Cost      = (($AppGPRST['LineTotal']+$AppGPRST['LineVatSum']) - $AppGPRST['LineProfit']) / $AppGPRST['Quantity'];
			$LineTotal = $AppGPRST['UnitPrice'] * $AppGPRST['Quantity'];

			$AppGP .= "<tr$trClass>";
				$AppGP .= "<td class='text-center'><input type='checkbox' class='form-check-input' $SP disabled></td>";
				$AppGP .= "<td>".$AppGPRST['ItemCode']." | ".$AppGPRST['ItemName']."</td>";
				$AppGP .= "<td class='text-center'>".$AppGPRST['WhsCode']."</td>";
				$AppGP .= "<td class='text-right'>".number_format($AppGPRST['Quantity'],0)."</td>";
				$AppGP .= "<td>".$AppGPRST['UnitMsr']."</td>";
				// $AppGP .= "<td class='text-right'>".number_format($Cost,2)."</td>";
				$AppGP .= "<td class='text-right'>".number_format($AppGPRST['UnitPrice'],3)."</td>";
				$AppGP .= "<td class='text-right'>".number_format($LineTotal,2)."</td>";
				$AppGP .= "<td class='text-right'>".number_format($AppGPRST['LineProfit'],2)."</td>";
				$AppGP .= "<td class='text-center'>".number_format($GP,2)."</td>";
				$AppGP .= "<td class='text-right'>".number_format($AppGPRST['MT_In'],2)."</td>";
				$AppGP .= "<td class='text-right'>".number_format($AppGPRST['MT_Out'],2)."</td>";
			$AppGP .= "</tr>";
		}

		if($SUMProfit > 0) {
			$SUMGP = ($SUMProfit / $SUMPrice) * 100;
		} else {
			$SUMGP = 0;
		}
		
		$AppGP .= "<tr class='table-active text-primary' style='font-weight: bold;'>";
			$AppGP .= "<td colspan='6' class='text-center'>รวมทั้งหมด</td>";
			$AppGP .= "<td class='text-right'>".number_format($SUMPrice,2)."</td>";
			$AppGP .= "<td class='text-right'>".number_format($SUMProfit,2)."</td>";
			$AppGP .= "<td class='text-center'>".number_format($SUMGP,2)."</td>";
			$AppGP .= "<td colspan='2' class='text-center'>&nbsp;</td>";
		$AppGP .= "</tr>";

		$arrCol['view_approveGP'] = $AppGP;
	}
