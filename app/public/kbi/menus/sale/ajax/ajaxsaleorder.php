...
if ($_GET['p'] == "GetAddress") {
	$sql = "SELECT 
				T0.CardCode, T0.AdresType AS 'AddressType', T0.Address AS 'AddressID', (T0.Street+' '+T0.Block+' '+T0.City+' '+ISNULL(CAST(T0.ZipCode AS VARCHAR),'')) AS 'FullAddress',
...
if ($_GET['p'] == "GetSlpCode") {
	$SlpSQL = "SELECT T0.SlpCode, T0.SlpName, T0.U_Dim1 AS 'MainTeam', T0.Active FROM OSLP T0 WHERE T0.SlpCode > 0 AND T0.Fax IS NULL
				ORDER BY
				CASE
					WHEN T0.U_Dim1 = 'MT1' THEN 1
					WHEN T0.U_Dim1 = 'EXP' THEN 2
					WHEN T0.U_Dim1 = 'MT2' THEN 3
					WHEN T0.U_Dim1 = 'TT2' THEN 4
					WHEN T0.U_Dim1 = 'TT1' THEN 5
					WHEN T0.U_Dim1 = 'OUL' THEN 6
					WHEN T0.U_Dim1 = 'ONL' THEN 7
					WHEN T0.U_Dim1 = 'EI1' THEN 8
					WHEN T0.U_Dim1 = 'MKT' THEN 9
					WHEN T0.U_Dim1 = 'DMN' THEN 10
				ELSE 11 END, T0.U_name_end, T0.SlpName ASC";
	$SlpQRY = SAPSelect($SlpSQL);
	...
	$tempTeam = "";
	while($SlpRST = odbc_fetch_array($SlpQRY)) {
		...
		$outputSlp .= "<option value='".$SlpRST['SlpCode']."'$dis_opt>".conutf8($SlpRST['SlpName'])."</option>";
...
if ($_GET['p'] == "GetItemDetail") {
	...
	if(ChkRowSAP($HisSQL) > 0) {
		$HisQRY   = SAPSelect($HisSQL); // PERz
		...
		$History = "<p class=\"text-center text-muted\">ไม่พบข้อมูลการขาย</p>";
	}
	...
	if(ChkRowDB($QtaSQL) > 0) {
		...
		$Quota .= "<table class='table table-bordered table-hover' style='font-size: 14px;'>";
	...
	*/
	$CostSQL = "SELECT CASE WHEN T0.LastPurDat = '2022-12-31' OR T0.LastPurDat IS NULL THEN ISNULL(T1.LastPurPrc,T0.LastPurPrc) ELSE T0.LastPurPrc END  AS 'PriceAfVAT' FROM OITM T0 LEFT JOIN KBI_DB2022.dbo.OITM T1 ON T0.ItemCode = T1.ItemCode WHERE T0.ItemCode = '".$_POST['ItemCode']."'";
	// $CostSQL = "SELECT TOP 1 CASE WHEN T0.LstEvlPric > 0 THEN T0.LstEvlPric WHEN T0.LastPurPrc > 0 THEN T0.LastPurPrc ELSE 0 END AS 'PriceAfVAT' FROM OITM T0 WHERE T0.ItemCode = '".$_POST['ItemCode']."'";
	// if(CHKRowSAP8($CostSQL) > 0) {
	...
	// $CostQRY = conSAP8($CostSQL);	
	$CostRST = odbc_fetch_array($CostQRY);
	$arrCol['CXST'] = conData("A".(($CostRST['PriceAfVAT'])*1.07));
	...
	if(strlen($DocSubstr[0]) == 3) {
		...
		$GetItemSQL = 
			"SELECT
				...
				T1.GrandPrice, T1.Line_Disc1, T1.Line_Disc2, T1.Line_Disc3, T1.Line_Disc4, T1.Line_Disc5, T1.UnitPrice
		...
		if($Rows == 0) {
			...
			while($GetItemRST = mysqli_fetch_array($GetItemQRY)) {
				...
				if ($GetItemRST['Line_Disc5'] != NULL && $GetItemRST['Line_Disc5'] != "" && $GetItemRST['Line_Disc5'] != 0.00) {
					$Discount = number_format($GetItemRST['Line_Disc1'], 2) . "-" . number_format($GetItemRST['Line_Disc2'], 2) . "-" . number_format($GetItemRST['Line_Disc3'], 2) . "-" . number_format($GetItemRST['Line_Disc4'], 2) . "-" . number_format($GetItemRST['Line_Disc5'], 2);
				} elseif ($GetItemRST['Line_Disc4'] != NULL && $GetItemRST['Line_Disc4'] != "" && $GetItemRST['Line_Disc4'] != 0.00) {
				...
				$CostSQL = "SELECT CASE WHEN T0.LastPurDat = '2022-12-31' THEN ISNULL(T1.LastPurPrc,T0.LastPurPrc) ELSE T0.LastPurPrc END  AS 'PriceAfVAT' FROM OITM T0 LEFT JOIN KBI_DB2022.dbo.OITM T1 ON T0.ItemCode = T1.ItemCode WHERE T0.ItemCode = '".$GetItemRST['ItemCode']."'";
				$CostQRY = SAPSelect($CostSQL);

				// $CostSQL = "SELECT TOP 1 CASE WHEN T0.LstEvlPric > 0 THEN T0.LstEvlPric WHEN T0.LastPurPrc > 0 THEN T0.LastPurPrc ELSE 0 END AS 'PriceAfVAT' FROM OITM T0 WHERE T0.ItemCode = '".$GetItemRST['ItemCode']."'";
				// if(CHKRowSAP8($CostSQL) > 0) {
				// 	$CostQRY = conSAP8($CostSQL);	
				// } else {
				// 	$CostQRY = SAPSelect($CostSQL);
				...
				if(!isset($CostRST['PriceAfVAT'])) {
					...
					$CXSTTotal = ($CostRST['PriceAfVAT']*1.07)*$GetItemRST['Quantity'];
				}
...
if($_GET['p'] == "SaveDraft") {
	...
	if($OrderEntry == "") {
		...
		/* DELETE APPROVE STAGE */
		$DeleteSQL = "DELETE FROM apporder WHERE DocEntry = $OrderEntry";
		MySQLDelete($DeleteSQL);

		$TotalRow = $_POST['TotalRow'];
		$No = 0;
		for($r = 1; $r <= $TotalRow; $r++) {
			if(isset($_POST['ItemRow_'.$r]) && $_POST['ItemRow_'.$r] != "") {
				$GetName = explode("|",$_POST['ItemName_'.$r]);
...
function IndexCol($txt) {
	$IndexCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($txt);
	return $IndexCol;
}

if($_GET['p'] == 'ImportExcel') {
	$txtFirstNum = $_POST['txtFirstNum'];
	$txtSumNum = $_POST['txtSumNum'];
	$InItemCode = ($_POST['txtItemCode'] != "") ? IndexCol($_POST['txtItemCode']) : "";
	$InBarCode = ($_POST['txtBarCode'] != "") ? IndexCol($_POST['txtBarCode']) : "";
	$InQuantity = ($_POST['txtQuantity'] != "") ? IndexCol($_POST['txtQuantity']) : "";
	$InDiscount = ($_POST['txtDiscount'] != "") ? IndexCol($_POST['txtDiscount']) : "";
	$InPrice = ($_POST['txtPrice'] != "") ? IndexCol($_POST['txtPrice']) : "";

	if(isset($_FILES['txtFile']['name'])) {
		$Totals = $_FILES['txtFile']['name'];
		$FileProcess = explode(".",basename($_FILES['txtFile']['name']));
		$countProcess = count($FileProcess);
		if($countProcess == 2){
			$FileExt = $FileProcess[1];
		} else {
	...
	move_uploaded_file($_FILES["txtFile"]["tmp_name"],"../../../../FileImport/SOrderImport.".$FileExt);
	if($FileExt == 'xls') {
	...
	$spreadsheet = $reader->load("../../../../FileImport/SOrderImport.".$FileExt);
	$sheet = $spreadsheet->getSheet($spreadsheet->getFirstSheetIndex());
	$data = $sheet->toArray();
	$getData = array(); $r = 0; $c = 0;
	$Error['BarCode'] = ""; $Error['ItemCode'] = "";
			...
			if($r <= $txtSumNum) {
				...
				foreach ($cell as $value) {
					...
					switch ($c) {
						case $InBarCode: 
							$Name = "ItemCode"; 
							$SQL = "SELECT T0.ItemCode FROM OITM T0 WHERE (T0.BarCode = '$value' OR T0.BarCode2 = '$value' OR T0.BarCode3 = '$value')";
							if(CHKRowDB($SQL) != 0) {
								if(CHKRowDB($SQL) > 1) {
									$SQL .= " LIMIT 1";
									$Multi = "Y";
								...
								$SQL = "SELECT T0.ItemCode FROM OITM T0 WHERE T0.CodeBars = '$value'";
								if(ChkRowSAP($SQL) > 1) {
									$SQL = "SELECT TOP 1 T0.ItemCode FROM OITM T0 WHERE T0.CodeBars = '$value'";
									$Multi = "Y";
								}
								$QRY = SAPSelect($SQL);
								$RST = odbc_fetch_array($QRY);
							}
							if((isset($RST['ItemCode']))) {
								$value = $RST['ItemCode'];
							}else{
								$Error['BarCode'] .= ($Error['BarCode'] != "") ? ", ".$r : $r;
								$value = "";
							}
						break;
						case $InItemCode: 
							$Name = "ItemCode";
							$SQL = "SELECT T0.ItemCode FROM OITM T0 WHERE T0.ItemCode = '$value'";
							if(CHKRowDB($SQL) != 0) {
								if(CHKRowDB($SQL) > 1) {
									$SQL .= " LIMIT 1";
									$Multi = "Y";
							...
							if((isset($RST['ItemCode']))) {
								...
								$Error['ItemCode'] = ($Error['ItemCode'] != "") ? ", ".$r : $r;
								$value = "";
							}
						break;
						case $InQuantity: $Name = "Quantity"; break;
						case $InDiscount: $Name = "Discount"; break;
						case $InPrice: $Name = "Price"; break;
					}
					if($Name != "") {
						$getData[$r][$Name] = $value;
					}
					$c++;
				}
				$getData[$r]['Multi'] = $Multi;
			}
		}
	}
	$arrCol['Error'] = $Error;
