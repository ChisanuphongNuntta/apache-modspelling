...
// รายงานการเก็บเงิน
	if ($_GET['a'] == 'CollectingMoney') {
		...
		if ($_POST['Team'] != 'all') {
			...
			while ($result = odbc_fetch_array($sqlQRY)) {
				if ($TeamName != $result['Memo']) {
					...
					if ($TeamName != 'f14ebccb93d03ac5de9fe0683d19No01'){
		...
		// 			${"TeamName_".$TeamID} = $resultukey['FullName'];
		// 		} else {
		// 			${"TeamName_".$TeamID} = 'Error retrieving name';
		// 		}
		// 	} else {
		// 		${"TeamName_".$TeamID} = 'ส่วนกลาง TT2'; //new fix 12/09/2024 by Bom
		// 	}
		// }
	
		// $key = "TeamName_".$TeamID."_".$result['Group'];
		// if (!isset(${$key})) {
		// 	${$key} = 0; // Initialize if not set
		// }
		// ${$key} += $result['Amount'];
