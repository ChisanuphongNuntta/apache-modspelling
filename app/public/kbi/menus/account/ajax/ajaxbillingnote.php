...
if($_GET['a'] == 'CallData') {
	if((substr($_POST['CardCode'],0,3) == 'BI-')) {
		$sql = "SELECT TransID, CreateDate, CardCode, DocNum, CreateUkey FROM note_billing WHERE DocNum = '".$_POST['CardCode']."' AND DocStatus = 'A'";
		$ChkR = CHKRowDB($sql);
		if($ChkR != 0) {
		...
		$sql = "SELECT DISTINCT DocNum FROM note_billing WHERE DocNum LIKE '$Prefix%' ORDER BY CreateDate DESC, DocNum DESC";
		$chkR = ChkRowDB($sql);
		if($chkR == 0) {
			$DocNum = $Prefix."0001";
		} else {
	...
	if($ChkData == "Y") {
		$sql = "SELECT OCRD.[CardCode],OCRD.[CardName],OCRD.[LicTradNum],
				...
				WHERE OCRD.[CardCode] = '".$CardCode."' AND (CRD1.[AdresType] = 'B')";
		$sqlQRY = SAPSelect($sql);
		...
		if($alert == 0) {
			...
			while($resultBill = odbc_fetch_array($sqlQRY)) {
				...
				$billRefNo[$x]   = conutf8($resultBill['RefNo']);
			...
			$output = "";
			for($i = 1; $i <= $x; $i++) {
				$Chk = CHKRowDB("SELECT * FROM note_billing WHERE TransID = '".$billTransID[$i]."' AND DocStatus = 'A'");
				if($Chk == 0) {
					$rowClass = "fw-bold";
				...
				$output .= "<tr class='$rowClass'>
								...
								<td class='text-center'>
									<input type='checkbox' class='form-check-input' style='cursor: pointer;' name='chkid_".$billTransID[$i]."_".$billLineID[$i]."' id='chkid_".$billTransID[$i]."_".$billLineID[$i]."' onclick=\"chkRow('".$billTransID[$i]."','".$billRemark[$i]."','".$billLineID[$i]."')\" $ChkDis>
								</td>
							</tr>";
			}
			$sum = $i-1;
...
if($_GET['a'] == 'AddData') {
	$ChkRow = CHKRowDB("SELECT * FROM note_billing WHERE TransID = '".$_POST['TransID']."' AND LineNum = '".$_POST['LineNum']."' AND DocNum = '".$_POST['DocNum']."'");
	if($ChkRow == 0){
		if($_POST['Chk'] == 1) {
 			$sql1 = "SELECT DISTINCT
						CASE
							WHEN T0.TransType = 13 THEN (SELECT P0.CardCode FROM OINV P0 WHERE P0.DocNum = T0.Ref1)
							ELSE (SELECT P0.CardCode FROM ORIN P0 WHERE P0.DocNum = T0.Ref1)
						END AS 'CardCode',
						CASE
							WHEN T0.TransType = 13 THEN (SELECT P0.CardName FROM OINV P0 WHERE P0.DocNum = T0.Ref1)
							ELSE (SELECT P0.CardName FROM ORIN P0 WHERE P0.DocNum = T0.Ref1)
						END AS 'CardName'
					 FROM JDT1 T0
					 WHERE T0.TransID = ".$_POST['TransID']." AND T0.Account = '1121-01'";
			$sqlQRY = SAPSelect($sql1);
			$resultBill = odbc_fetch_array($sqlQRY);

			$INSERT = "INSERT INTO note_billing 
		...
		if($_POST['Chk'] == 0) {
			$DELETE = "DELETE FROM note_billing WHERE TransID = '".$_POST['TransID']."' AND DocNum = '".$_POST['DocNum']."' AND LineNum = '".$_POST['LineNum']."'";
			MySQLDelete($DELETE);
		}
	}
}
...
if($_GET['a'] == 'CallBI') {
	$sql = "SELECT DocNum FROM note_billing WHERE DocStatus = 'A' AND TransID = '".$_POST['TransID']."' AND LineNum = '".$_POST['LineNum']."'";
	$result = MySQLSelect($sql);
	$arrCol['DocNum'] = $result['DocNum'];
}

if($_GET['a'] == 'Save') {
	$ChkRow = CHKRowDB("SELECT * FROM note_billing WHERE DocNum = '".$_POST['DocNum']."'");
	if($ChkRow > 0) {
