	...
	*/
	...
	switch($LvCode) {
		...
		//case "LV052":
			...
			$UserShow = " AND (T2.UkeyReq = '".$_SESSION['ukey']."' OR T2.UkeyReq = '$LvCode') ";
		break;
		case "LV038":
			//case "LV052":
				$UserShow = " AND T2.UkeyReq = '$LvCode' ";
	...
	if ($LvCode == 'LV038'){
		$SQLWhr = " AND T2.UkeyReq = 'f14ebccb93d03ac5de9fe0683d195e10'" ;
	}
			...
			SELECT A0.*,
				...
				FROM (
					...
					WHERE
						...
						(T0.DraftStatus = 'N' AND T0.DocStatus = 'P' AND T0.AppStatus = 'P' AND T0.CANCELED = 'N' AND (T0.SlpCode IN (251, 291, 296) OR T6.DeptCode = '$DeptCode') AND T2.ResultApp ='0' $UserShow)
				) A0
		) B0
		WHERE B0.Shown = 'A'
	...
	if($Rows > 0 && $ChkRow == "N") {
		...
		while ($ShowData = mysqli_fetch_array($getList)){
			...
			*/
			if ($DocEntry != $ShowData['DocEntry'] ){
				...
				$DrafList['QQst'][$DrafList['DocEntry'][$ax]] = $ShowData['QQst'];
		...
		for ($i=1;$i<=$ax;$i++){
			...
			if($DrafList['QQst'][$DrafList['DocEntry'][$i]] == "Y") {
			...
			$output.=  "<tr>
							...
							<td class='text-left'>".$DrafList['CardCode'][$DrafList['DocEntry'][$i]]." ".$DrafList['CardName'][$DrafList['DocEntry'][$i]]." $QQtxt</td>
...
if ($_GET['a'] == 'approv'){
	...
	$sql2 = 
		"SELECT
			T0.CardCode, CASE WHEN T0.CreateDate <= '2022-12-31' THEN T1.CreateDate ELSE T0.CreateDate END 'FirstBillDate'
		FROM OCRD T0
		LEFT JOIN KBI_DB2022.dbo.OCRD T1 ON T0.CardCode = T1.CardCode
		WHERE T0.CardCode = '".$DataCus['CardCode']."'";
	$qry2 = SAPSelect($sql2);
	while($rst2 = odbc_fetch_array($qry2)) {
		$FirstBill = $rst2['FirstBillDate'];
	}
	/*
	...
	*/
	...
	$sql1 = "SELECT T1.Descr AS BillAddTxt,T2.Descr AS SaveTxt,T3.PymntGroup AS TextCR,T0.CreditLine
			 FROM OCRD T0
		...
		if($_POST['Mobile'] == "true") {
			...
			"<tr>
				<td class='pb-0'>
					...
					if($_SESSION['ukey'] != $rowApp['UkeyReq'] && $rowApp['UkeyReq'] != 'LV036') {
	...
	$arrCol['Crlimit'] = number_format($DataSAP['CreditLine']);

	$arrCol['OrderRemark'] = $DataCus['Comments'];
	$arrCol['FristBill'] = FullDate($FirstBill);
		...
		switch ($TbModal) {
			...
			case 6: 
				$sql = "SELECT  T0.CHQ_DateReturn,T0.CHQ_No,T0.CHQ_Amount,
							(SELECT SUM(X1.Amount) FROM chq_detail X1 WHERE X1.DocNum = T0.DocNum ) AS toPaid,
							(SELECT X2.DatePaid FROM chq_detail X2 WHERE X2.DocNum = T0.DocNum ORDER BY DatePaid DESC LIMIT 1) AS LastDate,
							(SELECT X3.Remark FROM chq_detail X3 WHERE X3.DocNum = T0.DocNum ORDER BY DatePaid DESC LIMIT 1) AS Remark
						FROM chq_return T0
...
if ($_GET['a'] == 'gptab'){
	...
	while($result = mysqli_fetch_array($sqlQRY)) {
		...
		$output .= "<tr>
						...
						<td class='text-right'><a href='javascript:void(0);' onclick='ViewPur(\"".$result['ItemCode']."\")'>".number_format(((($result['LineTotal']+$result['LineVatSum'])-$result['LineProfit'])/$result['Quantity']),2)."</a></td>
...
if($_GET['a'] == 'GetViewPur') {
	$ItemCode = $_POST['ItemCode'];
	$SQL = "
		SELECT
		(T0.LstEvlPric*1.07) AS 'SAPCost', (CASE WHEN T0.LastPurDat = '2022-12-31' THEN T1.LastPurPrc ELSE T0.LastPurPrc END * 1.07)  AS 'LastPurPrc'
		FROM OITM T0
		LEFT JOIN KBI_DB2022.dbo.OITM T1 ON T0.ItemCode = T1.ItemCode
		WHERE T0.ItemCode = '$ItemCode'";
	$QRY = SAPSelect($SQL);
	$result = odbc_fetch_array($QRY);
	$Data = "ต้นทุนเฉลี่ย (SAP) : ".number_format($result['SAPCost'],2)." บาท\nต้นทุนเข้าล่าสุด : ".number_format($result['LastPurPrc'],2)." บาท";
	$arrCol['Data'] = $Data;
}
