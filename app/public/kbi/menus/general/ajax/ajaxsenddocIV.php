...
if($_GET['p'] == "GetList") {
	...
	$SAPSQL =
		...
		WHERE ((T1.BeginStr IN ('IV-','IC-','HA-') OR (T1.BeginStr = '' OR T1.BeginStr IS NULL)) AND (YEAR(T0.DocDate) = $filt_year AND MONTH(T0.DocDate) = $filt_month)) $getSAPWhr
	...
	if($SapROW == 0) {
		...
		while($SAPRST = odbc_fetch_array($SAPQRY)) {
			...
			$arrCol['BD_'.$i]['NumAtCard']  = conutf8($SAPRST['NumAtCard']);
			$arrCol['BD_'.$i]['DocNum']     = $SAPRST['DocNum'];
			$arrCol['BD_'.$i]['DocDate']    = date("d/m/Y",strtotime($SAPRST['DocDate']));
			$arrCol['BD_'.$i]['CardCode']   = conutf8($SAPRST['CardCode'])." | ".conutf8($SAPRST['CardName']);
			...
			$StatusSQL = "SELECT T0.*, T1.* FROM billsr T0 LEFT JOIN logistic T1 ON T1.logiID = T0.logi_ukey WHERE T0.DocEntry = $DocEntry";
			...
			if($StRow > 0) {
				...
				$LoadSQL = "SELECT DISTINCT DATE(T0.OutTime) AS 'OutTime' FROM logi_detail T0 WHERE T0.BillEntry = $DocEntry AND T0.BillType = 'OINV' ORDER BY T0.OutTime DESC LIMIT 1";
...
if($_GET['p'] == "Approve") {
	...
	$ChkSQL = "SELECT T0.DocEntry FROM billsr T0 WHERE T0.DocEntry = $DocEntry";
	$Rows   = ChkRowDB($ChkSQL);
	if($Rows == 0) {
		$LoadSQL = "SELECT DISTINCT DATE(T0.OutTime) AS 'OutTime' FROM logi_detail T0 WHERE T0.BillEntry = $DocEntry AND T0.BillType = 'OINV' ORDER BY T0.OutTime DESC LIMIT 1";
		$LoadRST = MySQLSelect($LoadSQL);
		$LoadDate = $LoadRST['OutTime'];
		$DocDateSQL = "SELECT TOP 1 T0.DocDate FROM OINV T0 WHERE T0.DocEntry = $DocEntry";
		$DocDateQRY = SAPSelect($DocDateSQL);
		$DocDateRST = odbc_fetch_array($DocDateQRY);
		$DocDate    = $DocDateRST['DocDate'];
		$InsertSQL = "INSERT INTO billsr SET DocEntry = $DocEntry, DocDate = '$DocDate', Load_Date = '$LoadDate'";
		// echo $InsertSQL;
		MySQLInsert($InsertSQL);
	}

	$UpdateSQL  = "UPDATE billsr SET ";
	switch($SaveType) {
		case "ChkLogi": $UpdateSQL .= "logi_App = $Approve, logi_ukey = '$UpdateUkey', logi_Date = NOW()"; $chk = " logi_App "; break;
		case "ChkWhse": $UpdateSQL .= "Whs_App  = $Approve, Whs_ukey  = '$UpdateUkey', Whs_Date  = NOW()"; $chk = " Whs_App ";break;
		case "ChkAcnt": $UpdateSQL .= "Acc_App  = $Approve, Acc_ukey  = '$UpdateUkey', Acc_Date  = NOW()"; $chk = " Acc_App ";break;
	}
	...
	$sql1 = "SELECT DocEntry FROM billsr WHERE $chk = '1' AND DocEntry = $DocEntry";
	if (CHKRowDB($sql1) >= 1){
		echo "ส่งเลขที่เอกสารสำเร็จ";
	}else{
		echo "เกิดปัญหากรณาส่งใหม่";
	}
