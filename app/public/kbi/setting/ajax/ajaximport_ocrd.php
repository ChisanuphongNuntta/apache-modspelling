...
if($_GET['p'] == "ImportCard") {
    ...
    if($SAPRow == 0) {
        ...
        if($ERFRow > 0) {
            ...
            // if($ID) {
            //     $file_path = "../../../json/OCRD.json";
            //     if(file_exists($file_path)) {
            //         $getFile = file_get_contents($file_path);
            //         $arrJSON = json_decode($getFile, true);
            //         $arrJSON[] = array(
            //                         "CardCode" => $CardCode,
            //                         "CardName" => $CardName,
            //                         "CardType" => $CardType,
            ...
            //     } else {
            //         $GetJSONSQL = "SELECT T0.CardCode, T0.CardName, T0.CardType, T0.CardStatus FROM OCRD T0 WHERE T0.CardCode != '' and T0.CardName != '' ORDER BY T0.CardCode ASC";
            //         if(CHKRowDB($GetJSONSQL) > 0) {
            //             $GetJSONQRY = MySQLSelectX($GetJSONSQL);
            //             $ArrJSON = array();
            //             $i = 0;
            //             while($GetJSONRST = mysqli_fetch_array($GetJSONQRY)) {
            //                 $arrJSON[$i]['CardCode'] = $GetJSONRST['CardCode'];
            //                 $arrJSON[$i]['CardName'] = $GetJSONRST['CardName'];
            //                 $arrJSON[$i]['CardType'] = $GetJSONRST['CardType'];
            //                 $arrJSON[$i]['CardStatus'] = $GetJSONRST['CardStatus'];
            //                 $i++;
            //             }
            //             file_put_contents($file_path, json_encode($arrJSON, JSON_UNESCAPED_UNICODE));
            //         }
        ...
        if($arrCol['Status'] == "SUCCESS") {
            $EditSQL = "SELECT T0.CardCode, T0.CardName, CONCAT(T0.Lat,', ',T0.Lon) AS 'GPS', T0.CardType, T0.CardStatus FROM OCRD T0 WHERE T0.CardCode = '$CardCode' LIMIT 1";
            ...
            $arrCol['txt_CardType']   = $EditRST['CardType'];
