...
class clear_db{
	...
	function MySQL_SelectX($SQLcommand){
		$objQuery=mysql_query($SQLcommand);
		return $objQuery;
	}
	...
	function my_sql_show_rows3($field,$table,$event){
		if($event != NULL){
			$objQuery=mysql_query("SELECT ".$field." FROM ".$table." WHERE ".$event);
		}else{
			$objQuery=mysql_query("SELECT ".$field." FROM ".$table);
	...
	function MySQL_UpdateX($SQLcommand) {
		return mysql_query($SQLcommand);
	}
	function my_sql_insert($table,$set){
		return mysql_query("INSERT INTO ".$table." SET ".$set);
	}
	function MySQL_InsertX($SQLcommand) {
		return mysql_query($SQLcommand);
	}
