...
if ($CHkWork == 1){
    $sql1 = "SELECT DATE(DocDueDate) AS DueDate FROM picker_soheader WHERE ID = ".$ID;
    $DueDate = MySQLSelect($sql1);//$DueDate['DueDate']
    ...
    switch ($PickType['PickDay']){
        ...
        case 'A3' : // บิลเช้า   TT จัดวันนี้
            ...
            $sql2 = "SELECT T0.TableID,
                    ...
                    FROM checkertable T0 WHERE T0.TableID NOT IN (4)
                    ORDER BY ItemCount,Bill LIMIT 1";  
            // echo $sql1."<br/><br/>";
            // echo $sql2;
        ...
        case 'B2' : // MT ล่วงหน้า
            $sql1 = "SELECT T0.uKey,
                            IFNULL ((SELECT SUM(X0.ItemCount) FROM picker_soheader X0 WHERE X0.UkeyPicker = T0.uKey AND X0.TeamCode LIKE 'MT%' AND DATE(X0.DocDueDate) = '".dateSQL($DueDate['DueDate'])."'),0)  AS ItemCount,
                            IFNULL ((SELECT COUNT(X1.ID) FROM picker_soheader X1 WHERE X1.UkeyPicker = T0.uKey AND X1.TeamCode LIKE 'MT%' AND DATE(X1.DocDueDate) = '".dateSQL($DueDate['DueDate'])."' ),0)  AS Bill 
                    FROM users T0
                    WHERE T0.uKey IN ".$uKeyList." ORDER BY ItemCount,Bill LIMIT 1";  

            $sql2 = "SELECT T0.TableID,
                            IFNULL ((SELECT SUM(X0.ItemCount) FROM picker_soheader X0 WHERE X0.TablePacking = T0.TableID AND X0.TeamCode LIKE 'MT%' AND DATE(X0.DocDueDate) = '".dateSQL($DueDate['DueDate'])."' ),0)  AS ItemCount,
                            IFNULL ((SELECT COUNT(X1.ID) FROM picker_soheader X1 WHERE X1.TablePacking = T0.TableID AND X1.TeamCode LIKE 'MT%' AND DATE(X1.DocDueDate) = '".dateSQL($DueDate['DueDate'])."' ),0)  AS Bill 
                    FROM checkertable T0 WHERE T0.TableID NOT IN (4) 
                    ORDER BY ItemCount,Bill LIMIT 1";  
/*
$sql1 = "SELECT T0.uKey,
IFNULL ((SELECT SUM(X0.ItemCount) FROM picker_soheader X0 WHERE X0.UkeyPicker = T0.uKey AND X0.TeamCode LIKE 'MT%' AND DATE(X0.DocDueDate) = DATE(NOW()) ),0)  AS ItemCount,
IFNULL ((SELECT COUNT(X1.ID) FROM picker_soheader X1 WHERE X1.UkeyPicker = T0.uKey AND X1.TeamCode LIKE 'MT%' AND DATE(X1.DocDueDate) = DATE(NOW()) ),0)  AS Bill 
FROM users T0
WHERE T0.uKey IN ".$uKeyList." ORDER BY ItemCount,Bill LIMIT 1";  

$sql2 = "SELECT T0.TableID,
IFNULL ((SELECT SUM(X0.ItemCount) FROM picker_soheader X0 WHERE X0.TablePacking = T0.TableID AND X0.TeamCode LIKE 'MT%' AND DATE(X0.DocDueDate) = DATE(NOW()) ),0)  AS ItemCount,
IFNULL ((SELECT COUNT(X1.ID) FROM picker_soheader X1 WHERE X1.TablePacking = T0.TableID AND X1.TeamCode LIKE 'MT%' AND DATE(X1.DocDueDate) = DATE(NOW()) ),0)  AS Bill 
...
*/
            ...
            if ($PickType['PickDay'] == 'A4'){
                ...
                $WeekEND2 = 1;
                $Pick3Date =  date("Y-m-d",strtotime("-2 days",strtotime($DueDate['DueDate'])));
