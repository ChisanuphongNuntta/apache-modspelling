...
$FixBill = "T0.DocNum IN ('".$_POST['DocNum']."')";
...
while ($DataNew = mysql_fetch_object($getNewDoc)){
    switch($DataNew->PickDay){
        ...
        case 'A4' :// MT อีก3 วันส่ง
            $sqlItem = "SELECT X0.user_key AS UserKey,X0.ItemCount,X0.BillCount
                        FROM (SELECT T0.user_key,
                                     (SELECT SUM(P0.ItemCount) FROM picker_soheader P0 WHERE (MONTH(P0.OriDate) = MONTH(NOW()))  AND P0.UkeyPicker = T0.user_key AND P0.TeamCode LIKE 'MT%') AS ItemCount,
                                     (SELECT COUNT(P1.SODocEntry) FROM picker_soheader P1 WHERE (MONTH(P1.OriDate) = MONTH(NOW()))  AND P1.UkeyPicker = T0.user_key AND P1.TeamCode LIKE 'MT%') AS BillCount        
                              FROM picker_online T0
                              WHERE DATE(T0.DateOnline) = DATE(NOW()) AND T0.Status = 1
                              ) X0
                        ORDER BY X0.ItemCount,X0.BillCount 
            ...
            $sqlTbl = "SELECT X0.TableFix as tblPack,X0.ItemCount,X0.BillCount
                       FROM (SELECT T0.TableFix,
                                    (SELECT SUM(P0.ItemCount) FROM picker_soheader P0 WHERE (MONTH(P0.OriDate) = MONTH(NOW())) AND P0.TablePacking = T0.TableFix AND P0.TeamCode LIKE 'MT%' ) AS ItemCount,
                                    (SELECT COUNT(P1.ItemCount) FROM picker_soheader P1 WHERE (MONTH(P1.OriDate) = MONTH(NOW())) AND P1.TablePacking = T0.TableFix AND P1.TeamCode LIKE 'MT%' ) AS BillCount 
                            FROM tableOnline T0
                            WHERE DATE(T0.DateOnline) = DATE(NOW()) AND T0.Status = 1
                            ) X0
                       ORDER BY X0.ItemCount,X0.BillCount 
