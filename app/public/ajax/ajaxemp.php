...
if ($_POST['fun'] == 1 ){
    $getdata->my_sql_update("allwhs","AllFree = 'Y'","LocationRack = '".$_POST['LocRack']."'");
}else{
    $output = "B";
}
$mysql = "SELECT LocationRack FROM allwhs WHERE AllFree = 'Y' ORDER BY LocationRack";
$getRack = $getdata->MySQL_SelectX($mysql);
while ($FreeRack = mysql_fetch_object($getRack)){
    $output .= " <tr>";
    $output .= "    <td>".$FreeRack->LocationRack."</td>";
    $output .= " </tr>";
}
