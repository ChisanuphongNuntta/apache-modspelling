...
$MsSQL = " SELECT '".$_SERVER['REMOTE_ADDR']."' AS 'Query IP',Z0.* 
           FROM (SELECT DISTINCT  T0.[DocEntry],T0.[DocDate],T0.[DocTime],T0.[DocDueDate],T0.[CreateDate],T1.[BeginStr],T0.[DocNum],T0.DocStatus,T0.[CardCode],T0.[CardName],T0.[OwnerCode],
                                  T4.U_Dim1 AS 'xCh',
                                  ...
                                  (SELECT COUNT(P0.LineNum) FROM RDR1 P0 WHERE P0.DocEntry = T0.DocEntry) AS xDetail,
                                   ...
                                   CASE WHEN T4.U_Dim1 = 'MT1' AND T1.BeginStr LIKE 'SB%' THEN 'N' ELSE 'Y' END AS OnAdd
                 FROM  ORDR T0 
                 ...
                 WHERE T0.CreateDate >= '".$dateOnStart."' AND (T0.CreateDate >= DATEADD(month,-1,GETDATE())) AND ( 
                       ...
                       T0.[SlpCode] NOT IN (20,123,124,125,126))  ".$ORDR." ".$FixBill." ) Z0
          WHERE Z0.OnAdd = 'Y' 
          ORDER BY Z0.xCh DESC,Z0.DocEntry";
//echo  $MsSQL."/n"
